...
def _run_scheduler_pipeline(req: GenerateScheduleRequest):
    ...
    return scheduler, dept_timetables, conflicts, metrics
...
def generate_schedule(req: GenerateScheduleRequest):
    try:
        scheduler, dept_timetables, conflicts, metrics = _run_scheduler_pipeline(req)
...
@app.post("/export/pdf")
def export_pdf(req: GenerateScheduleRequest):
    try:
        scheduler, dept_timetables, _, _ = _run_scheduler_pipeline(req)
        semester = str(req.operating_rules.get("semester", 4))
        pdf_bytes = export_to_pdf(
            dept_timetables,
            department="SRMIST Vadapalani",
            semester=semester,
            global_matrix=scheduler.global_matrix
        )
        return Response(
            content=pdf_bytes,
            media_type="application/pdf",
            headers={"Content-Disposition": f'attachment; filename="SRMIST_VDP_Timetable_Sem{semester}.pdf"'}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def export_excel(req: GenerateScheduleRequest):
    try:
        scheduler, dept_timetables, _, metrics = _run_scheduler_pipeline(req)
        semester = str(req.operating_rules.get("semester", 4))
